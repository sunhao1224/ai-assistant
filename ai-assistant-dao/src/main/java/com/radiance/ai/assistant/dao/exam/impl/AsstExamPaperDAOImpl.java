 ...
 */
@Repository
public class AsstExamPaperDAOImpl implements AsstExamPaperDAO {

    @Resource
    private AsstExamPaperMapper asstExamPaperMapper;

    @Override
    public List<AsstExamPaperDO> list(AsstExamPaperQuery data) {
        if (data.getDataStatus() == null) {
            data.setDataStatus(DataStatusEnum.NORMAL.getValue());
        }
        return asstExamPaperMapper.list(data);
    }

    @Override
    public int insertBatch(List<AsstExamPaperDO> list) {
        list.forEach(e -> {
            if (e.getType() == null) {
        ...
        return asstExamPaperMapper.insertBatch(list);
    }

    @Override
    public int updateBatch(List<AsstExamPaperDO> list) {
        list.forEach(e -> {
            if (e.getUpdateTime() == null) {
                e.setUpdateTime(DateUtil.getNow());
            }
        ...
        return asstExamPaperMapper.updateBatch(list);
    }

    @Override
    public int removeBatch(List<Long> list) {
        return asstExamPaperMapper.removeBatch(list);
    }
