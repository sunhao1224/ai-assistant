package com.radiance.ai.assistant.dao.exam.impl;

import com.radiance.ai.assistant.common.utils.DateUtil;
import com.radiance.ai.assistant.dao.exam.AsstExamAnswerDAO;
import com.radiance.ai.assistant.dao.exam.AsstExamPaperDAO;
import com.radiance.ai.assistant.domain.dos.exam.AsstExamAnswerDO;
import com.radiance.ai.assistant.domain.enums.DataStatusEnum;
import com.radiance.ai.assistant.domain.enums.exam.AsstExamDimensionTypeEnum;
import com.radiance.ai.assistant.domain.query.exam.AsstExamAnswerQuery;
import com.radiance.ai.assistant.domain.query.exam.AsstExamPaperQuery;
import com.radiance.ai.assistant.mapper.exam.AsstExamAnswerMapper;
import com.radiance.ai.assistant.mapper.exam.AsstExamPaperMapper;
import org.springframework.stereotype.Repository;

import javax.annotation.Resource;
import java.util.List;

/**
 * 考试维度数据层接口实现类
 *
 * @author juncong.sh@alibaba-inc.com
 * @date 2024/9/25 14:32
 * @since 1.0.0
 */
@Repository
public class AsstExamAnswerDAOImpl implements AsstExamAnswerDAO {

    @Resource
    private AsstExamAnswerMapper asstExamAnswerMapper;

    @Override
    public List<AsstExamAnswerDO> list(AsstExamAnswerQuery data) {
        if (data.getDataStatus() == null) {
            data.setDataStatus(DataStatusEnum.NORMAL.getValue());
        }
        return asstExamAnswerMapper.list(data);
    }

    @Override
    public int insertBatch(List<AsstExamAnswerDO> list) {
        list.forEach(e -> {
            if (e.getType() == null) {
                e.setType(AsstExamDimensionTypeEnum.COMMON.getValue());
            }
            if (e.getDataStatus() == null) {
                e.setDataStatus(DataStatusEnum.NORMAL.getValue());
            }
            if (e.getCreateTime() == null) {
                e.setCreateTime(DateUtil.getNow());
            }
            if (e.getUpdateTime() == null) {
                e.setUpdateTime(DateUtil.getNow());
            }
            if (e.getFinishTime() == null) {
                e.setFinishTime(DateUtil.getNow());
            }
        });
        return asstExamAnswerMapper.insertBatch(list);
    }

    @Override
    public int updateBatch(List<AsstExamAnswerDO> list) {
        list.forEach(e -> {
            if (e.getUpdateTime() == null) {
                e.setUpdateTime(DateUtil.getNow());
            }
            if (e.getFinishTime() == null) {
                e.setFinishTime(DateUtil.getNow());
            }
        });
        return asstExamAnswerMapper.updateBatch(list);
    }

    @Override
    public int removeBatch(List<Long> list) {
        return asstExamAnswerMapper.removeBatch(list);
    }

    @Override
    public int deleteBatch(List<Long> list) {
        return asstExamAnswerMapper.deleteBatch(list);
    }

    @Override
    public int deleteByAsstExamBankId(List<Long> list) {
        return asstExamAnswerMapper.deleteByAsstExamBankId(list);
    }

}
