...
import com.radiance.ai.assistant.dao.exam.AsstExamCommentDAO;
...
import com.radiance.ai.assistant.mapper.exam.AsstExamCommentMapper;
import com.radiance.ai.assistant.mapper.exam.AsstExamPaperMapper;
 ...
 */
@Repository
public class AsstExamCommentDAOImpl implements AsstExamCommentDAO {

    @Resource
    private AsstExamCommentMapper asstExamCommentMapper;

    @Override
    public List<AsstExamCommentDO> list(AsstExamCommentQuery data) {
        if (data.getDataStatus() == null) {
            data.setDataStatus(DataStatusEnum.NORMAL.getValue());
        }
        return asstExamCommentMapper.list(data);
    }

    @Override
    public int insertBatch(List<AsstExamCommentDO> list) {
        list.forEach(e -> {
            if (e.getType() == null) {
        ...
        return asstExamCommentMapper.insertBatch(list);
    }

    @Override
    public int updateBatch(List<AsstExamCommentDO> list) {
        list.forEach(e -> {
            if (e.getUpdateTime() == null) {
                e.setUpdateTime(DateUtil.getNow());
            }
        ...
        return asstExamCommentMapper.updateBatch(list);
    }

    @Override
    public int removeBatch(List<Long> list) {
        return asstExamCommentMapper.removeBatch(list);
    }

    @Override
    public int deleteBatch(List<Long> list) {
        return asstExamCommentMapper.deleteBatch(list);
    }
