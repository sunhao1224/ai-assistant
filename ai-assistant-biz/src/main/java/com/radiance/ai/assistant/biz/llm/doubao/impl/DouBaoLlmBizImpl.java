...
public class DouBaoLlmBizImpl implements DouBaoLlmBiz {
    ...
    public String promptGenerate(String content) {
        // 推荐生成 prompt
        String systemContent = "- 你叫 Prompt 完善者，每当我给出一个写好的 prompt，你会迅速将其转化为适合大模型的详细 prompt。你思维敏捷，能准确理解我的需求，用丰富的描述扩展词语的内涵，为大模型提供精准的引导。\n" +
...
//        return douBaoLlm.chat(systemContent, "以下内容都是prompt：" + content);
    ...
    public static void main(String[] args) {
        String a = "角色：你是一个专业的现代建筑学课程老师 ，现在需要你批改一套现代建筑学的现代建筑学试卷，需要按照以下【任务要求】执行。\n" +
                "【评分规则】：\n" +
                "- 如果【题目类型】是主观题，学生答案需要围绕（维度,一级指标,二级指标）内容以及【参考答案】展开，必须包含的核心字段有：创新，越贴近得分越高。\n" +
                "- 如果【题目类型】是客观题，学生答案只需要与参考答案进行匹配，相似度越高，得分越高，参考答案中的关键名字不能写错，写错需要扣分。\n" +
                "\n" +
                "（维度,一级指标,二级指标）：\n" +
                "(功能性, 空间布局, 功能分区合理性), (功能性, 功能完整度, 功能适应性), (功能性, 流线组织, 人流、物流、车流合理性), (安全性, 结构安全, 结构稳定性), (安全性, 消防安全, 消防设施完善度), (安全性, 人身安全, 防滑措施), (美观性, 造型设计, 立面美观度), (美观性, 色彩搭配, 色彩协调性), (美观性, 景观融合, 建筑与周边环境协调性), (经济性, 投资预算, 投资估算准确性), (经济性, 成本控制, 材料成本), (环保性, 节能措施, 保温隔热性能), (环保性, 环保材料, 绿色建材使用率), (社会性, 地域文化, 建筑地域特色), (社会性, 人文关怀, 无障碍设施)\n" +
                "\n" +
                "【任务要求】：\n" +
