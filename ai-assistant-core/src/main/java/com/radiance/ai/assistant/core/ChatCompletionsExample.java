...
//        final ChatMessage userMessage = ChatMessage.builder().role(ChatMessageRole.USER).content("常见的十字花科植物有哪些？").build();
        final ChatMessage systemMessage = ChatMessage.builder().role(ChatMessageRole.SYSTEM).content("- 你是一位专业的语言分析大师，能准确判断该题目答案和学生作答的相似度并给出评分和最终得分。当我给出答案和学生作答后，你会迅速分析其中的核心词汇，通过复杂的算法得出相似度评分，评分为100。\n" +
                "- 如果两个句子的核心词完全不同，你会明确指出差异并给出较低评分；若有一定相似性，你会详细解释相似之处及给出合理评分。打分可以适当放宽\n" +
                "- 使用json格式返回，相似度评分字段的名称为：score，原因字段的名称为：desc").build();
//        final ChatMessage systemMessage = ChatMessage.builder().role(ChatMessageRole.SYSTEM).content("你是一个可以给出专业prompt的人工智能助手").build();
//        final ChatMessage userMessage = ChatMessage.builder().role(ChatMessageRole.USER).content("价值塑造、自强不息、厚德载物和价值塑性、自强不息、厚德载物").build();
        final ChatMessage userMessage = ChatMessage.builder().role(ChatMessageRole.USER).content("给出一个可以判断文字型题目的判定规则的prompt").build();
